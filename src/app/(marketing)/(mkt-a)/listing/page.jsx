...
const Page = () => {
  ...
  useEffect(() => {
    if (roleFromUrl && roleData[roleFromUrl]) {
  ...
  const handleTabClick = (role) => {
    ...
    router.push(`/listing?${newSearchParams.toString()}`);
    setActiveRole(role); 
  };
    ...
    <div>
      <Navbarr>
        <div className="flex flex-row justify-between items-center">
          ...
          <Image src="/categoryNavbar.svg" alt="" width={300} height={100} />
        </div>
      </Navbarr>
        ...
        {Object.keys(roleData).map((role) => (
          <button
            ...
            className={`relative px-4 py-2 m-10 cursor-pointer font-semibold transition-all duration-300 ${
              activeRole === role ? "text-[#9D71C6]" : "hover:text-[#9D71C6]"
            }`}
          >
            <span
